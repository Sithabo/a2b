...
export default function ReceiptsScreen() {
  ...
  const renderOpenCard = (shipment: Shipment) => (
    <ModernShipmentCard key={shipment.id} shipment={shipment} status="waiting" />
  );

  const renderActiveCard = (shipment: Shipment) => (
    <ModernShipmentCard key={shipment.id} shipment={shipment} status="found" onPress={() => router.push('/active-delivery')} />
  );
          ...
          {hasOpen ? (
            ...
            <>
              <ModernShipmentCard 
                key="mock1" 
                status="waiting" 
                shipment={{ id: "8F2C99X", createdAt: new Date(Date.now() - 15 * 60000).toISOString(), pickup: "Kampala, Makindye", delivery: "Jinja, Industrial Area" }} 
              />
              <ModernShipmentCard 
                key="mock2" 
                status="waiting" 
                shipment={{ id: "P2AL01Z", createdAt: new Date(Date.now() - 60 * 60000).toISOString(), pickup: "Entebbe, Airport Road", delivery: "Kampala, City Center" }} 
              />
            </>
          )}
        </View>
        ...
        <View style={styles.activeSection}>
          ...
          {hasActive ? (
            ...
            <ModernShipmentCard 
              key="mock3" 
              status="found" 
              shipment={{ id: "V99MZLQ", createdAt: new Date(Date.now() - 120 * 60000).toISOString(), pickup: "Kampala, Makindye", delivery: "Jinja, Industrial Area" }} 
